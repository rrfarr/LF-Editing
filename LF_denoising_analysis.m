function LF_denoising_analysis(dn_method,sig,out_flag)
...
[lf_names, datasets] = read_configuration('config\denoising.cfg');
...
elseif strcmp(dn_method,'bm3d')
    ...
    out_LF_foldername  = sprintf('RESULTS/denoising/sig%d/LF/bm3d/',sig);
elseif strcmp(dn_method,'vbm4d')
    fprintf('Evaluating the performance of VBM4D\n');
    out_filename = sprintf('RESULTS/denoising/sig%d/vbm4d.csv',sig);
    out_img_foldername = sprintf('RESULTS/denoising/sig%d/centre_view/vbm4d/',sig);
    out_LF_foldername  = sprintf('RESULTS/denoising/sig%d/LF/vbm4d/',sig);
...
for n = 1:N
    ...
    elseif strcmp(dn_method,'vbm4d')
        % Use vbm4d denoising
        RQ_LF = LF_vbm4d_denoising(LQ_LF,sig);
    elseif strcmp(dn_method,'bm3d-epi')
