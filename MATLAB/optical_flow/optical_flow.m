function [u_,v_] = optical_flow(LR_LF)
...
siftflow_foldername = 'MATLAB/optical_flow/';

if ~exist(siftflow_foldername,'dir')
    % Create the siftflow optical flow folder
    mkdir(siftflow_foldername);
    
    % Derive the siftflow filename
    siftflow_filename = [siftflow_foldername,'SIFTflow.zip'];
    ...
    % Unzip the folder
    unzip(siftflow_filename,siftflow_foldername);
